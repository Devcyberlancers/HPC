...
def gpu_worker(gpu_id: int, shared_dict):
    try:
        ...
        while True:
            ...
            total_updates = len(FIELDS_TO_USE) * MC_RUNS_PER_GPU * NX * NZ * STEPS_PER_CYCLE
            shared_dict[f"gpu{gpu_id}_cell_updates_per_sec"] = float(total_updates / max(compute_sec, 1e-8))
...
<body>
    ...
    <div class="gpu-grid">
        ...
        <div class="gpu-box">
            <h3>GPU {{ g.id }}</h3>
            <div class="small">Compute time: {{ g.compute_sec }}</div>
